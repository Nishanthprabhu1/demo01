/* script.js - Jewels-Ai Atelier: Drive + WhatsApp + BG Removal + Actual Names */
...
/* --- UPDATED PRELOADER (WITH AUTO-TRANSPARENCY) --- */
    ...
    if (!PRELOADED_IMAGES[type]) {
        ...
        const promises = files.map(file => {
            return new Promise((resolve) => {
                ...
                img.onload = () => {
                    // Try to remove background once loaded
                    try {
                        const transparentImg = removeWhiteBackground(img);
        ...
        loadingStatus.style.display = 'block';
        loadingStatus.textContent = "Processing Transparency...";
        
        const loadedImages = await Promise.all(promises);
        PRELOADED_IMAGES[type] = loadedImages.filter(img => img !== null);
...
function removeWhiteBackground(sourceImg) {
    ...
    const THRESHOLD = 230; // 0-255 (Higher = strict white only)
...
function captureToGallery() {
  ...
  let itemName = "Jewels-Ai Look"; let itemFilename = "Jewels-Ai_look.png";
